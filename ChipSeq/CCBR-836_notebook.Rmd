...
regions=read.delim("mouse_prom_1000_rev_sort_chr.txt",header=FALSE,sep="\t")
prom_regions=GRanges(regions$V2,IRanges(regions$V3,regions$V4))
#reg.counts<-regionCounts(bam.files,param=readParam(pe="none"),prom_regions)

```
```{r Run ahead of time and save files}

#Set up parameters to do window counts, using dedup, discard repeats:
repeats=read.table("mm9-blacklist.bed")
repeats.gr=GRanges(repeats$V1,IRanges(repeats$V2,repeats$V3))
param=readParam(minq=10, dedup=TRUE,discard=repeats.gr)

#win=list()  
#collected=list()
#Get the profiles around 
for (bam in bam.files) {
  windowed <- windowCounts(bam, spacing=50, width=50, param=param, filter=20)
  win[[bam]] <- windowed
  rwsms <- rowSums(assay(windowed))
  maxed <- findMaxima(rowRanges(windowed), range=1000, metric=rwsms)
  collected[[bam]] <- profileSites(bam, rowRanges(windowed)[maxed], 
          param=param, weight=1/rwsms[maxed])}

save(win,file="win.RData")  #Takes long time to run - run 1x and save.

profile(win,collected,"csaw.all.png")

```
```{r Profile plot function}

#Function to take the aggregate of the signal:
profile <- function(w,x,y){
xranged <- as.integer(names(x[[1]]))
ymax=max(unlist(lapply(x ,function(x) max(x))))/3
plot.new()
png(y)
plot(xranged, (x[[1]]/w[[1]]$totals)*10e6, type="l", col="blue", 
     xlim=c(-1000, 1000), ylim=c(0,ymax),lwd=2,xlab="Distance (bp)",
     ylab="Relative coverage per base")
...
lines(xranged, (x[[6]]/w[[6]]$totals)*10e6, col="cyan", lwd=2)
...
###Taking TSS regions from all ChIP sites using csaw windows:

TSSregions.list=list()
for (i in 1:6){
rwsms <- rowSums(assay(win[[i]]))
maxed <- findMaxima(rowRanges(win[[i]]), range=1000, metric=rwsms)
allregions=rowRanges(win[[i]])[maxed]
peakTSS=suppressWarnings(findOverlaps(allregions,prom_regions))
TSSregions.list[[i]]=allregions[queryHits(peakTSS),]
}

TSSregall=GRanges()
for (j in 2:5){
TSSregall=GenomicRanges::union(TSSregall,TSSregions.list[[j]])
}

collected.tss=list()
for (bam in bam.files){
collected.tss[[bam]]=profileSites(bam, TSSregall, param=param, weight=1/rwsms[maxed])
}

profile(win,collected.tss,"csaw.tss.png")

```
```{r}

#Take published Nanog peaks and profile csaw peaks:
#http://www.sciencedirect.com/science/article/pii/S1934590915003045
...
Nanogpeaks=read.table("GSE56312_mES_Nanog_Serum.NARROWPEAK")
Nanog.df=data.frame(chrom=Nanogpeaks$V1,start=Nanogpeaks$V2,end=Nanogpeaks$V3)
NanogGR=as(Nanog.df,"GRanges")
NanogGR

collected.nanog=list()
for (bam in bam.files){
collected.nanog[[bam]]=profileSites(bam, NanogGR, param=param, weight=1/rwsms[maxed])
}

profile(win,collected.nanog,"csaw.nanog.png")

#Take published Nanog peaks and find overlapping csaw peaks:

Nanogregions.list=list()
for (bam in bam.files) {
rwsms <- rowSums(assay(win[[bam]]))
maxed <- findMaxima(rowRanges(win[[bam]]), range=1000, metric=rwsms)
allregions=rowRanges(win[[bam]])[maxed]
Nanogregions.list[[bam]]=suppressWarnings(subsetByOverlaps(allregions,NanogGR))
}

Nanogregall=GRanges()
for (j in 2:5){
Nanogregall=GenomicRanges::union(Nanogregall,Nanogregions.list[[j]])
}

collected.Nanogcsaw=list()
for (bam in bam.files){
collected.Nanogcsaw[[bam]] <- profileSites(bam, Nanogregall, 
          param=param, weight=1/rwsms[maxed])
}

profile(win,collected.Nanogcsaw,"csaw.nanogpeaks.png")
...
##Makeplot makes plot of number of peaks vs number of reads:
...
macsTSSregions.list=list()
allmacsTSSregions=GRanges()

for (i in 1:6){
rwsms <- rowSums(assay(w[[i]]))
maxed <- findMaxima(rowRanges(w[[i]]), range=1000, metric=rwsms)
allregions=rowRanges(w[[i]])[maxed]
macsTSSregions.list[[i]]=subsetByOverlaps(allregions,TSSpeaks.macs)
allmacsTSSregions=union(allmacsTSSregions,macsTSSregions.list[[i]])
#macsTSSregions.list[[i]]=allregions[queryHits(peakTSS),]
}
...
allmacs.TSSpeaks=subsetByOverlaps(allmacspeaks,prom_regions)
TSSpeaks.macs=subsetByOverlaps(prom_regions,allmacspeaks)

comppeaks=list(
  findmacs=unique(TSSpeaks.macs),
  findcsaw=unique(allmacs.TSSpeaks)
)

k=c(1,2)
venn.names=as.character(names(comppeaks)[k])
res <- makeVennDiagram(Peaks=comppeaks[k],
                        NameOfPeaks=venn.names)
makevenn(res,venn.names[1],venn.names[2],"Nanog_KO_Venn_MACS.png")


collected2=list()
for (curbam in bam.files) {
  collected2[[curbam]] <- profileSites(curbam, allmacsTSSregions, 
                  param=param)}
#  collected2[[curbam]] <- profileSites(curbam, allmacs.TSSpeaks, 
#                  param=param)}
profile(collected2,"MACS.TSS.2.png")
...
macspeaks2=list(
  macs=allmacs.TSSpeaks,
  csaw=allmacsTSSregions
)

k=c(1,2)
venn.names=as.character(names(macspeaks2)[k])
res <- makeVennDiagram(Peaks=macspeaks2[k],
                        NameOfPeaks=venn.names)
makevenn(res,venn.names[1],venn.names[2],"Nanog_KO_Venn_MACS.png")
...
groupedmacspeaks=list(
  ...
  WT.KO=allmacspeaks,
  Nanog.narrow=NanogGR,
  Nanog.csaw=Nanogregall)
...
res2 <- makeVennDiagram(Peaks=groupedmacspeaks[k],
                        NameOfPeaks=venn.names)
makevenn(res2,venn.names[1],venn.names[2],venn.names[3],
         "Nanogpks_macs_WT_KO_Pooled_Venn.png")

k=c(4,5)
venn.names=as.character(names(groupedmacspeaks)[k])
res2 <- makeVennDiagram(Peaks=groupedmacspeaks[k],
                        NameOfPeaks=venn.names)
makevenn(res2,venn.names[1],venn.names[2],
         "Nanogpks_Venn.png")
