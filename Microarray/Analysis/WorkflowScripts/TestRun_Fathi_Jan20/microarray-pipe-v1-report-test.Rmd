---
title: "CCBR402 Microarray project Report"
...
marray_pipeline_mm_affy2st= function(project,inputdir,outdir,phenofile,contrasfile) {
  ...
  library(geneplotter)
  library(mogene20sttranscriptcluster.db, quietly = TRUE)
  ## parameters
  ...
  ## check names od samples are in the same order in cels and pd
  
  if (length(which((rownames(pd)==cels)==T)) != length(cels))
  {
    cat("Please sort the phenotype file by Sample Name : ..Quitting!!!\n")
    return (-1)
  }
  
  celfiles <- suppressMessages(read.celfiles(cels, phenoData=pd))
 
  ## check ## pData(celfiles@protocolData)
  ##
    ...
    
  {
    ...
    cat("group : ",labfacs[i], " after normalization\n")
  ...
  heatmap.2(mat, trace="none", margin=c(10,10),main="Heatmap")
  # 
  ...
  for (i in 1:nb)
  {
    ...
    plotMA(dataf, main=paste("MAplot: ",cons[i],sep=""))
    # dev.off()
...
marray_pipeline_mm_affy2st("CCBR-402","/Users/elloumif/Documents/projects/microarray-techdev","/Users/elloumif/Documents/projects/microarray-techdev","ccbr402Sampledata.txt","contrastsList.txt")
```
